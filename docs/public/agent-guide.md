...
A node's `agents.worker.model`/`agents.supervisor.model` overrides the run-wide model for that stage only; `envctl run show` and the dashboard Conversation panel show the effective model per stage and role.

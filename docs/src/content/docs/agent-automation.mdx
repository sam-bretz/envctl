...
`-C` changes the worktree envctl operates on, and the feature name follows that worktree's branch. Combine with `--feature` to address an environment whose worktree is gone.
...
An MCP server exposing status, logs, exec, seed, snapshot, and promote is on the [Roadmap](/docs/roadmap/). Until then, agents call the CLI.

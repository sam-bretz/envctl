...
Both scripts must be on `PATH` and need `jq`. See [Install](/envctl/install/).
...
Every command accepts `--json`, and `status --json` includes the env file lines, so an agent can read connection details in one call. See [Agent automation](/envctl/agent-automation/).

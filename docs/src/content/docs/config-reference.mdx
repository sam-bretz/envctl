...
| `limits.run_tokens` | int | `12000000` | Counted-token ceiling for the whole run, across every revision. `-1` disables it. See below for what counts. |
...
Cache reads are almost all of it. On every tool call (reading a file, running a test) the agent re-reads its whole conversation from the prompt cache, so a coding session that grows to 200K tokens and makes 130 tool calls reports about 25M cache-read tokens. They cost about a tenth of other input, so the ceiling counts them at `limits.cache_read_weight` (10% by default) and everything else in full. A complete feature run on envctl itself reports 30M to 40M tokens and counts about 5M.

`envctl run show` and the dashboard show the counted total against the ceiling and the reported total, for example `tokens 5.3M counted of 12.0M (44%) · 40.2M reported · cache reads 38.8M at 10% · API-equivalent $6.50`. The dashboard colors it at 80% and 100%. Usage of jobs still running is estimated until each job reports its final count.
...
- The run shows `needs-attention` with the reason, for example `run counted 12.0M of its 12.0M token ceiling (limits.run_tokens; 95.1M tokens reported, cache reads at 10%)`.
...
For scale: dogfooding runs of envctl on itself reported 30M to 40M tokens each, about 5M counted, and $6 to $17 API-equivalent, including supervisor corrections and retries.

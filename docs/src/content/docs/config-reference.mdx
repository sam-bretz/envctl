...
The supervisor proposes variations only when it accepts the work, each with a name and a rationale, and proposes none when the approach taken is clearly right. They appear on the stage's Checkpoint in the dashboard and in `envctl run show --json`.

**Variations are recorded, not yet built.** Nothing runs them as separate approaches today. How that will work is in the [supervisor-proposed variations design](/envctl/design/supervisor-variations/).

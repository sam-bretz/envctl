...
`agents.worker.model` and `agents.supervisor.model` set the model each role's
...
Most of a run's tokens are cache reads: every tool call re-reads the agent's conversation so far, so long sessions in stages that read a lot of code cost the most. On envctl's own runs, one code attempt reported 26M of a run's 40M tokens. Ways to spend less:
...
- **Use a cheaper model where the work is verified anyway.** Code and QA workers are checked by their stage's checks and by an independent supervisor, so a faster model such as Sonnet is a good fit there. Keep the default model for Plan and Design, whose decisions the later stages build on, and for every supervisor:
  ...
  workflow:
    template: feature
    nodes:
      code: {agents: {worker: {model: sonnet}}}
      qa: {agents: {worker: {model: sonnet}}}
  ```

- **Use the `small` workflow for small changes.** Plan, Build and the approved change skip Task, Design and a separate QA stage. See [Workflows](#workflows-version-2-experimental).
- **Keep stages focused.** Prompts that name the files and commands to use save the exploring an agent otherwise does.
- **Watch the ceiling.** `limits.run_tokens` stops a run that runs away. Raising it with a rewind that only changes limits resumes where the run stopped.
...
- The run shows `needs-attention` with the reason, for example `run counted 20.0M of its 20.0M token ceiling (limits.run_tokens; 160.4M tokens reported, cache reads at 10%)`.
...
`tracker` links a run to an external issue and posts a captain's log comment to it as each workflow stage completes: what each stage decided or built, what was checked, and what is waiting on a person.

```yaml
tracker:
  provider: linear
  credential: file:/absolute/path/to/linear.token   # or env:LINEAR_API_KEY
```
...
The log posts one comment per event:

| Event | Comment |
| --- | --- |
| Stage completed | The stage, attempt, worker summary, supervisor review, checks, commits, token usage, and a cumulative "so far" section. |
| Awaiting approval | Which stage is waiting on a person. |
| Approved | Which attempt was approved. |
| Pull request opened | The URL, one line per repository for a run spanning several. |
| Rewound | The new revision, its target stage, and the reason or objective change. |
| Needs attention | The cause, such as the token ceiling or a publication failure. |

### Moving the issue's status

`tracker.mapping` moves the linked issue through your tracker's statuses as the run progresses, so the ticket stays in step without anyone updating it. It is optional; without it, statuses are never changed and only comments are posted.

The fastest way to write it is the wizard, which reads your team's real statuses and suggests a mapping:

```bash
envctl tracker setup
```

It asks for a credential **file path** — never the key itself, which it reads, checks, and never prints or writes — then your team, then a status for each event, chosen from a numbered list with sensible defaults already selected. It shows the YAML and the transitions a typical run would make before writing anything, and re-running it edits the existing mapping. It needs a terminal and refuses otherwise.

A worked Linear example, for a team with the default board:

```yaml
tracker:
  provider: linear
  credential: file:/Users/me/.config/envctl/linear.token
  mapping:
    default:
      run_started: In Progress
      awaiting_approval:
        approved-change: In Review
      pr_published: Done
      cancelled: Canceled
      rewound: In Progress
    small:
...
`mapping` is keyed by workflow name (`default` for the `workflow:` section), because named workflows need not share stages. Events:

| Event | Fires when | Keyed by stage |
| --- | --- | --- |
| `run_started` | The revision starts preparing or running | no |
| `stage_started` | A stage starts | yes |
| `stage_accepted` | A stage's result is accepted | yes |
| `awaiting_approval` | A stage waits for a person | yes |
| `approved` | A result is approved | no |
| `pr_published` | The approved change opens its pull request | no |
| `needs_attention` | The run needs attention | no |
| `cancelled` | The run is cancelled | no |
| `rewound` | A rewind starts a new revision | no |

An unknown event or stage name is rejected when the configuration loads. Status names are checked during Plan's readiness against your team's actual workflow states, without changing anything; an unknown status fails readiness and lists the valid ones.

Each transition applies exactly once, in order, across coordinator restarts, using the same durable receipts as comments. A transition waiting to retry holds back the ones after it, so a brief outage cannot leave the issue at an earlier status than the run reached. A tracker outage or rejected transition never blocks or fails the workflow; `envctl run show` reports pending and failed updates.

**Manual changes are overwritten.** If someone moves the issue by hand, the next mapped event moves it again: the mapping always applies, and the last write wins. This is not a free choice — "only move forward" would contradict `rewound`, since a rewind after `Done` has to be able to move the issue back to work. Leave an event unmapped if you would rather it never touch the status.

...
Status: decided 10 September 2026, to be implemented at the start of milestone 2, before any VM backend code.
...
`--env` replaces `--env`; `--env` stays as a deprecated alias for one release. Names follow the same slug rules as today: lowercase, `a-z0-9-`, at most 40 characters, unique per project prefix per backend.
...
There is still no server. `envctl env list` reconciles tags, labels, and the JSON documents.
...
This is a rename while milestone 1 is fresh and a migration after milestone 3 ships, which is why it goes first.

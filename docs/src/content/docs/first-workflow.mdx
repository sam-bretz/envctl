...
Runs are grouped by what they need from you: waiting on you, running, finished. Select a stage on the stage track to see its conversation, checkpoint, changes and tests. Artifacts open in a reading pane with Markdown rendered, and a run that is blocked or waiting for approval says so at the top with the next step. The same keys work there (`?` lists them).

...
Agents report a lot of tokens: a full feature run on envctl itself reports 30M to 40M, nearly all cache reads, which count at a tenth toward the ceiling (about 5M counted). `envctl run show` and the dashboard show each run's counted and reported totals, and every run has a ceiling of 12M counted tokens by default. At the ceiling, envctl stops the agents and waits for you. Set your own with `limits.run_tokens`, or cap the reported cost with `limits.run_cost_usd`. See [Token usage and ceilings](/envctl/config-reference/#token-usage-and-ceilings).

...
With no `credential`, Codex workers use `OPENAI_API_KEY` if it's set, otherwise your Codex login in `~/.codex/auth.json`. Run `codex login` first if you use a ChatGPT plan. A plan's usage limit stops running workers mid-stage, so check your remaining usage before a long run.

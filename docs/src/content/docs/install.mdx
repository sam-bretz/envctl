...
Codex workers need a long-lived credential too: a Codex access token, or an OpenAI API key. envctl refuses the ChatGPT login in `~/.codex/auth.json`. Its refresh token rotates like Claude's, and OpenAI says [not to share one `auth.json` across machines or concurrent jobs](https://learn.chatgpt.com/docs/auth/ci-cd-auth).

**Codex access token** (the `claude setup-token` equivalent). [Codex access tokens](https://learn.chatgpt.com/docs/enterprise/access-tokens) are available only in ChatGPT Business and Enterprise workspaces, and a workspace owner must first allow members to use them.

1. Create one at [chatgpt.com/admin/access-tokens](https://chatgpt.com/admin/access-tokens). Choose an expiry and rotate it like any automation secret.

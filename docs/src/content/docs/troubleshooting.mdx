...
Checks Docker/Compose, git, Lima, `gh`, the manifest, and (on macOS) the Xcode Command Line Tools in one pass, printing an exact one-line fix for anything wrong. It's the fastest way to tell whether a problem below is actually a machine setup issue. See [Doctor](/envctl/cli-reference/#doctor) for what each check means.
